...
class _HomeViewContent extends StatelessWidget {
  ...
  Widget _buildFilterRow(BuildContext context, HomeViewmodel viewModel) {
      ...
      child: Row(
        children: [
          ...
          if (viewModel.filters.hasActiveFilters) ...[
            const SizedBox(width: 8),
            _buildActiveFilterChips(context, viewModel),
          ],
        ],
      ),
    );
  ...
  Widget _buildActiveFilterChips(BuildContext context, HomeViewmodel viewModel) {
    final chips = <Widget>[];
    
    // Chip para limpiar todos los filtros
    chips.add(
      InputChip(
  ...
  void _showFilterSheet(BuildContext context, HomeViewmodel viewModel) {
    showModalBottomSheet(
      ...
      builder: (context) => FilterSheet(
        ...
        showRatingFilter: false, // No mostrar rating en pendientes
        onFiltersChanged: viewModel.setFilters,
      ),
    );
  }

  void _showSortOptions(BuildContext context, HomeViewmodel viewModel) {
    showModalBottomSheet(
      context: context,
  ...
  IconData _getEmptyIcon(HomeViewmodel viewModel) {
    if (viewModel.filters.hasActiveFilters) {
  ...
  String _getEmptyMessage(HomeViewmodel viewModel) {
    ...
    if (hasFilters && hasSearch) {
      ...
      return 'No hay restaurantes favoritos pendientes';
    } else if (hasSearch) {
      return 'No hay resultados para "${viewModel.searchQuery}"';
    }
    return 'No hay restaurantes pendientes';
  }
